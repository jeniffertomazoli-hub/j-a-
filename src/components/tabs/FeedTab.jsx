...
export default function FeedTab({ quemSouEu, settings }) {
  ...
  async function handleCurtir(post) {
    const curtidasAtuais = Array.isArray(post.curtidas) ? post.curtidas : [];
    const jaCurtiu = curtidasAtuais.includes(quemSouEu);
    ...
    if (!jaCurtiu) {
      confetti({
        ...
        spread: 50,
        origin: { y: 0.7 },
        colors: ['#FF4D97', '#FFD93D'],
      });
      toast.love(`Você curtiu a publicação de ${post.autor_nome}! ❤️`);
    }

    try {
      await alternarCurtidaPost(post.id, quemSouEu, curtidasAtuais);
      carregarPosts();
    } catch (err) {
      console.error(err);
    }
  ...
  async function handleAdicionarComentario(postId, comentariosAtuais = []) {
    ...
    try {
      ...
      toast.love('Comentário enviado com carinho! 💌');
      carregarPosts();
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="px-3 sm:px-6 pt-4 max-w-md mx-auto pb-16 animate-fadeIn">
      ...
      <p className="text-xs text-white/80 font-medium mb-4">
        Compartilhem fotos do dia a dia, pensamentos e momentos especiais 💜
      </p>

      {/* Barra Rápida de Criar Publicação */}
      <div className="card-brut p-3.5 mb-5 shadow-brut bg-white">
        <div className="flex items-center gap-2.5">
          <span className="w-9 h-9 rounded-full bg-yellow border-2 border-ink flex items-center justify-center text-lg shrink-0 shadow-brutsm">
            {meuEmoji}
          </span>
          <button
      ...
      {/* Feed de Publicações (Estilo Instagram do Casal) */}
      <div className="space-y-5">
        {posts.length === 0 ? (
          ...
          posts.map((post) => {
            ...
            const euCurti = curtidas.includes(quemSouEu);
              ...
              >
                ...
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2 min-w-0">
                    <span className="w-8 h-8 rounded-full bg-yellow border-2 border-ink flex items-center justify-center text-sm shrink-0 shadow-brutsm">
                      {autorEmoji}
                    </span>
                ...
                {/* Barra de Ações (Like e Comentário) */}
                <div className="flex items-center justify-between gap-2 mb-2 pt-1 border-t-2 border-ink/10">
                  <div className="flex items-center gap-3">
                    {/* Botão Curtir */}
                    <button
                      onClick={() => handleCurtir(post)}
                      className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full border-2 border-ink text-xs font-black transition active:scale-95 shadow-brutsm ${
                        euCurti
                          ? 'bg-pink text-white scale-102'
                          : 'bg-white text-ink hover:bg-pink/10'
                      }`}
                    >
                      <span>{euCurti ? '❤️' : '🤍'}</span>
                    ...
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 rounded-full border-2 border-ink text-xs font-black bg-white text-ink hover:bg-yellow/20 transition shadow-brutsm"
                    >
                      <span>💬</span>
                  ...
                  {/* Indicador de quem curtiu */}
                  {curtidas.length > 0 && (
                    <span className="text-[10px] font-bold text-ink/60">
                      {curtidas.includes('parceiro1') && curtidas.includes('parceiro2')
                        ? 'Curtido por vocês dois! 💜'
                        : curtidas.includes(quemSouEu)
                        ? 'Você curtiu'
                        : `${outroNome} curtiu ❤️`}
                    </span>
                  )}
                  ...
                  <div className="mt-3 pt-3 border-t-2 border-ink/10 space-y-2.5 animate-fadeIn">
                    ...
                    {comentarios.length === 0 ? (
                      <p className="text-[11px] text-ink/50 font-bold italic text-center py-1">
                        Nenhum comentário ainda. Deixe um recadinho! 💬
                      </p>
                    ) : (
                      <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                        {comentarios.map((com) => {
                          ...
                          return (
                            <div
                              key={com.id}
                              className="bg-ink/5 rounded-xl px-3 py-2 border border-ink/10 text-xs"
                            >
                              ...
                              <p className="text-ink/85 font-medium break-words">
                                {com.texto}
                              </p>
                            </div>
                          );
                    ...
                    <div className="flex gap-2 pt-1">
                      <input
                        ...
                        placeholder={`Comentar como ${meuNome}...`}
